...
function convertToSameType(aCommand, bCommand) {
    ...
    if (aCommand.type !== bCommand.type && bCommand.type.toUpperCase() !== 'M') {
        ...
        Object.keys(bCommand).forEach(bKey => {
            ...
            if (aValue === undefined) {
                if (( readFromBKeys as any ).includes(bKey)) {
                    aValue = bValue;
                } else {
        ...
        ( aConverted as any ).type = bCommand.type;
        aCommand = aConverted;
    }
...
function splitSegment(commandStart, commandEnd, segmentCount) {
    ...
    if (commandEnd.type === 'L' || commandEnd.type === 'Q' || commandEnd.type === 'C') {
        ...
        const copyCommand = ( Object as any ).assign({}, commandStart);
        ...
        segments = segments.concat(arrayOfLength(segmentCount - 1, null).map(() => copyCommand));
...
function extend(commandsToExtend, referenceCommands, excludeSegment) {
    ...
    const countPointsPerSegment = arrayOfLength(numReferenceSegments, null).reduce((accum, d, i) => {
    ...
    const extended = countPointsPerSegment.reduce((extended, segmentCount, i) => {
        ...
        if (i === commandsToExtend.length - 1) {
            const lastCommandCopies = arrayOfLength(segmentCount,
                ( Object as any ).assign({}, commandsToExtend[commandsToExtend.length - 1]));
